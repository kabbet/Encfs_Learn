...
namespace encfs {
    ...
    //the array.
    //It is caller's responsibility to make sure the output array is large
    //enough
    ...
    /*
        Same as changeBase2, except the output is written over the input data. The output
        is assumed to be large enough to accept the data.

        Uses the stack to store output values. Recurse every time a new value is to be written,
        then write the value at the tail end of the recursion.
     */
    static void changeBase2Inline(unsigned char* src, int srcLen, int src2Pow,
                                  int dst2Pow, bool outputParitialLastByte,
        ...
        if (srcLen != 0) {
            ...
            changeBase2Inline(src, srcLen, src2Pow, dst2Pow, outputParitialLastByte,
                              work, workBits, outLoc + 1);
            *outLoc = outVal;
        } else {
            *outLoc++ = outVal;

            if (outputParitialLastByte) {
                while (workBits > 0) {
                    *outLoc++ = work & mask;
                    work >>= dst2Pow;
                    workBits -= dst2Pow;
    ...
    void changeBase2Inline(unsigned char* src, int srcLen, int src2Pow, int dst2Pow,
            bool outputParitialLastByte) {
        changeBase2Inline(src, srcLen, src2Pow, dst2Pow, outputParitialLastByte, 0, 0, nullptr);
    }

    // character set for ascii b64:
    // ",-0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
    // a standard base64 (eg a64l doesn't use ',-' but use './'. We don't
...
#define INVALID 66
    ...
    const static char encodeLoopup[] = 
    ...
    std::string B64StandardEncode(const std::vector<unsigned char>& inputBuffer) {
        ...
        for (size_t idx = 0; idx < inputBuffer.size() / 3; idx++) {
            ...
            temp += (*cursor++) << 8;
            temp += (*cursor++);
            encodedString.append(1, encodeLoopup[(temp & 0x00FC0000) >> 18]);
            encodedString.append(1, encodeLoopup[(temp & 0x0003F000) >> 12]);
            encodedString.append(1, encodeLoopup[(temp & 0x00000FC0) >> 6]);
            encodedString.append(1, encodeLoopup[(temp & 0x0000003F)]);
        }
